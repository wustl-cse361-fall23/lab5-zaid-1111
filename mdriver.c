...
int main(int argc, char **argv)
{
    ...
    double secs, ops, tput, tput_geom, util;
    double avg_mm_util, avg_mm_geom_throughput = 0;
    double p1, p1_checkpoint;
    double p2_new, p2_new_checkpoint;
    double p2_best, p2_best_checkpoint;
    ...
    if (errors == 0) {
        ...
        /* p2_new - throughput index with new average */
        p2_new = (1.0 - UTIL_WEIGHT) *
            compute_scaled_score(avg_mm_geom_throughput,
                MIN_THROUGHPUT, MAX_THROUGHPUT);
        p2_new_checkpoint = (1.0 - UTIL_WEIGHT_CHECKPOINT) *
            compute_scaled_score(avg_mm_geom_throughput,
                MIN_THROUGHPUT_CHECKPOINT, MAX_THROUGHPUT_CHECKPOINT);

        /* p2_best - throughput index averages */
        p2_best = p2_new;
        p2_best_checkpoint = p2_new_checkpoint;


        perfindex = (p1 + p2_best) * 100.0;
        perfindex_checkpoint = (p1_checkpoint + p2_best_checkpoint) * 100.0;
